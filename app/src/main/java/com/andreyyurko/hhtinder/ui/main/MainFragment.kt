...
import com.andreyyurko.hhtinder.ui.MainActivityViewModel
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.getCVState().collect { state ->
                    when (state) {
                        is MainViewModel.GetCVState.Ok -> {
                        ...
                        is MainViewModel.GetCVState.Error -> {
                            // TODO: обработка ошибки получения CV
                        }
                        is MainViewModel.GetCVState.Loading -> {
